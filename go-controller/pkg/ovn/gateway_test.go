...
func generateGatewayInitExpectedNB(testData []libovsdb.TestData, expectedOVNClusterRouter *nbdb.LogicalRouter, expectedNodeSwitch *nbdb.LogicalSwitch, nodeName string, clusterIPSubnets []*net.IPNet, hostSubnets []*net.IPNet,
	l3GatewayConfig *util.L3GatewayConfig, joinLRPIPs, defLRPIPs []*net.IPNet, noSnat bool) []libovsdb.TestData {
	...
	if !noSnat {
		for _, subnet := range clusterIPSubnets {
			natUUID := libovsdbops.GenerateNamedUUID()
		...
		// TODO(flaviof): Depending on config mode, we may want all the nats
		if len(natUUIDs) > 1 {
...
node4 chassis=912d592c-904c-40cd-9ef1-c2e5b49a33dd lb_force_snat_ip=100.64.0.4`,
	...
	ginkgo.It("creates an IPv4 gateway in OVN", func() {
		...
		noSnat := false
		expectedDatabaseState := generateGatewayInitExpectedNB(testData, expectedOVNClusterRouter, expectedNodeSwitch, nodeName, clusterIPSubnets, hostSubnets, l3GatewayConfig, joinLRPIPs, defLRPIPs, noSnat)
