...
func TestSyncServices(t *testing.T) {
	...
	tests := []struct {
...

		{
			...
			initialDb: []libovsdbtest.TestData{
				nodeLogicalSwitch(nodeA, []string{types.ClusterLBGroupName, types.ClusterSwitchLBGroupName}),
				nodeLogicalSwitch(nodeB, []string{types.ClusterLBGroupName, types.ClusterSwitchLBGroupName}),
				nodeLogicalRouter(nodeA, []string{types.ClusterLBGroupName, types.ClusterRouterLBGroupName}),
				nodeLogicalRouter(nodeB, []string{types.ClusterLBGroupName, types.ClusterRouterLBGroupName}),
		...
		},
		{
			...
			initialDb: []libovsdbtest.TestData{
				...
				nodeLogicalRouter(nodeA, []string{types.ClusterLBGroupName, types.ClusterRouterLBGroupName}, loadBalancerClusterWideTCPServiceName(ns, serviceName)),
				nodeLogicalRouter(nodeB, []string{types.ClusterLBGroupName, types.ClusterRouterLBGroupName}, loadBalancerClusterWideTCPServiceName(ns, serviceName)),
		...
		},
		{
			...
			initialDb: []libovsdbtest.TestData{
				...
				nodeLogicalSwitch(nodeA, []string{types.ClusterLBGroupName, types.ClusterSwitchLBGroupName}, loadBalancerClusterWideTCPServiceName(ns, serviceName)),
				nodeLogicalSwitch(nodeB, []string{types.ClusterLBGroupName, types.ClusterSwitchLBGroupName}, loadBalancerClusterWideTCPServiceName(ns, serviceName)),
				nodeLogicalRouter(nodeA, []string{types.ClusterLBGroupName, types.ClusterRouterLBGroupName}, loadBalancerClusterWideTCPServiceName(ns, serviceName)),
				nodeLogicalRouter(nodeB, []string{types.ClusterLBGroupName, types.ClusterRouterLBGroupName}, loadBalancerClusterWideTCPServiceName(ns, serviceName)),
				lbGroup(types.ClusterLBGroupName),
				lbGroup(types.ClusterSwitchLBGroupName),
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("%d_%s", i, tt.name), func(t *testing.T) {
			...
			for _, node := range defaultNodes {
				controller.nodeTracker.nodeIPV4Template.Value[node.chassisID] = node.nodeIPsStr[0]
			}
