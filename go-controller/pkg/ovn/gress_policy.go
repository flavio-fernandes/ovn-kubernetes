...
func (gp *gressPolicy) buildACLAllow(match, l4Match string, ipBlockCIDR int, aclLogging string) *nbdb.ACL {
	...
	policyTypeIndex := fmt.Sprintf("%d", gp.idx)
	...
	acl := libovsdbops.BuildACL(aclName, aclName, direction, match, action, types.OvnACLLoggingMeter, getACLLoggingSeverity(aclLogging), priority, aclLogging != "", externalIds)
	return acl
}
