...
set_default_params() {
  ...
  export OVN_SECONDARY_CNI_ENABLE=${OVN_SECONDARY_CNI_ENABLE:-false}
...
create_ovn_kubernetes() {
    ...
    helm install ovn-kubernetes . -f ${value_file} \
        ...
        --set global.emptyLbEvents=$(if [ "${OVN_EMPTY_LB_EVENTS}" == "true" ]; then echo "true"; else echo "false"; fi) \
          ...
          --set global.enableSecondaryCni=$(if [ "${OVN_SECONDARY_CNI_ENABLE}" == "true" ]; then echo "true"; else echo "false"; fi) \
          ${ovnkube_db_options}
}
